...
void load_item_topic_table(const string &item_topic_table_path, vector<int64_t> &topic_sum_table, vector<vector<WordCount>> &topic_words) {
...
public:
    ...
    static TopicTable create(int num_topics, const string &vocabulary_path, const string &item_topic_table_path) { 
        return TopicTable(num_topics, vocabulary_path, item_topic_table_path); 
    };
    ...
    std::vector<std::tuple<std::string, float>> show_topics(int topic_id, int k) {
        ...
        if (topic_id >= 0 && topic_id < _num_topics) {
            ...
            for (int i = 0; i < k; i++) {
                float prob = (_topic_words[topic_id][i].count * 1.0) / (_topic_sum_table[topic_id] + EPS);
...
PYBIND11_MODULE(topictable, m) {
    py::class_<familia::TopicTable>(m, "TopicTable").def(py::init(&familia::TopicTable::create))
